...
class OffensiveReflexAgent(ReflexCaptureAgent):
  ...
  def getFeatures(self, gameState, action):
    ...
    if len(foodList) > 0: # This should always be True,  but better safe than sorry
      ...
      if len(currentFoodList) > len(foodList) and min_ghost_distance > 2: #If we're going to eat food and the ghost is far away, +50
        features['eatFood'] = 1
        # print "a"
  ...
  def getWeights(self, gameState, action):
    ...
    return {'successorScore': 100, 'numInvaders': -1000, 'distanceToFood': -1, 'eatFood': 50, 'maybe_kill_range_ghost': -50 ,'kill_range_ghost': -100, 'stop': -100, 'reverse': -2, 'eat_range_ghost': 100, 'turn_around': -100, 'eat_dat_boi': 100, 'invaderDistance': -10}
